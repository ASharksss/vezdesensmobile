...
const CreateCardPage = () => {
	...
	return (
		<div className='createCard'>
			{addressOpen ?
				<FullScreenModal activeModal={addressOpen} setActiveModal={setAddressOpen}><Geoposition setShow={setAddressOpen}
					...
					<div className="createCard_characteristics">
						...
						{characterArray?.map((character, index) => {
							if (character.characteristic.required) {
								if (character.characteristic.typeCharacteristic.name === 'select') {
									return <SelectInput setSelectValue={setSelectValue} key={'select' + index} isRequired={true}
									                    data={character.characteristic} id={character.characteristicId}/>
								}
								if (character.characteristic.typeCharacteristic.name === 'checkbox') {
									return <CheckboxInput setCheckboxValue={setSelectValue} key={'checkbox' + index} isRequired={true}
									                      data={character.characteristic} id={character.characteristicId}/>
								}
						...
						{characterArray?.map((character, index) => {
							if (!character.characteristic.required) {
								...
								if (character.characteristic.typeCharacteristic.name === 'checkbox') {
									return <CheckboxInput setCheckboxValue={setSelectValue} key={'checkbox' + index} isRequired={false}
									                      data={character.characteristic} id={character.characteristicId}/>
								}
								if (character.characteristic.typeCharacteristic.name === 'enter') {
									return <EnterInput setEnterValue={setEnterValue} key={'enter' + index}
			...
			<div className="createPage_size">
				<div className="createPage_size-standart">
					...
					{selectedSize !== 'Стандарт' ? <BookingCalc selectedSize={selectedSize} setBookingEndDate={setBookingEndDate}
					...
					{preview.name !== '' &&
						<div className="upload_photo">
							...
							{preview.name === 'Премиум' ?
								...
								preview.name === 'ВИП' ? <UploadPhotoVip editedImage={previewImage} setEditedImage={setPreviewImage}/> :
			...
			<button
				...
				type='submit' disabled={!agreeOffers || !agreeRules || loading}>
